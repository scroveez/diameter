...
# $Id: radacct.cgi,v 1.14 2003/02/04 23:43:20 mikem Exp $
...
where USERNAME = '$username' and ACCTSTATUSTYPE='Stop'
order by TIME_STAMP";
                ...
                $framed_ip_address) = $sth->fetchrow)
	{
	    ...
	    my $hrefacctsessionid  = $acctsessionid;
            $hrefacctsessionid =~ s/ /%20/g;
	    ...
	    $table .= "<tr bgcolor=yellow><td><a href=$me?type=session_id&user=$username&session_id=$hrefacctsessionid&filename=$filename>$acctsessionid</a></td><td>$eventtime</td><td><a href=$me?type=framed_ip_address&framed_ip_address=$framed_ip_address&filename=$filename>$framed_ip_address</a></td><td>$time</td><td>$acctinputoctets</td><td>$acctoutputoctets</td><td>?</td><td>?</td></tr>\n";
	}
    }
    elsif ($DBFIACol && $type eq 'framed_ip_address')
    {
	my $q = "select USERNAME, ACCTSESSIONID, TIME_STAMP, 
ACCTSESSIONTIME,
ACCTINPUTOCTETS, ACCTOUTPUTOCTETS from $DBTableName 
where $DBFIACol = '$framed_ip_address' and ACCTSTATUSTYPE='Stop'
order by TIME_STAMP";
	my $sth = &prepareAndExecute($dbh, $q);
		...
		$acctinputoctets, $acctoutputoctets) = $sth->fetchrow)
	{
	    ...
	    $table .= "<tr bgcolor=yellow><td><a href=$me?type=user&user=$username&filename=$filename>$username</a></td><td><a href=$me?type=session_id&user=$username&session_id=$hrefacctsessionid&filename=$filename>$acctsessionid</a></td><td>$eventtime</td></td><td>$time</td><td>$acctinputoctets</td><td>$acctoutputoctets</td><td>?</td><td>?</td></tr>\n";
	}
