...
# $Backport-Id: EAP.pm,v 1.51 2010/03/04 07:31:12 mikem Exp $
...
sub authenticateUserEAP
{
	...
	elsif ($type == $Radius::EAP::EAP_TYPE_NAK)
	{
		...
		my $class = $self->getEAPClass($desired)
		    || return ($main::IGNORE, "Desired EAP type $desired not supported. This may be due to a broken client, or a problem loading the requuires EAP module");
		$p->{EAPType} = $desired;
	    ...
	    {
		return ($main::REJECT, "Desired EAP type $Radius::EAP::eap_type_to_name{$desired} ($desired) not permitted");
	    }
	}
	elsif ($type)
	{
